...
describe("cleanUserAgent", () => {
  it("cleans mac agent", () => {
    const initialMacAgent = `Mozilla/5.0 (Macintosh; Intel Mac OS X 11_6_0) AppleWebKit/537.36 (KHTML, like Gecko) Black Mask/${process.version} Chrome/${process.versions.chrome} Electron/${process.versions.electron} Safari/537.36`;
    expect(cleanUserAgent(initialMacAgent)).toEqual(expectedUserAgent);
  });

  it("cleans windows agent", () => {
    const initialWindowsAgent = `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Black Mask/${process.version} Chrome/${process.versions.chrome} Electron/${process.versions.electron} Safari/537.36`;
    expect(cleanUserAgent(initialWindowsAgent)).toEqual(expectedUserAgent);
  });

  it("cleans linux agent", () => {
    const initialWindowsAgent = `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Black Mask/${process.version} Chrome/${process.versions.chrome} Electron/${process.versions.electron} Safari/537.36`;
    expect(cleanUserAgent(initialWindowsAgent)).toEqual(expectedUserAgent);
  });
  ...
  it("does not change version numbers", () => {
    ...
    const initialAgent = `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Black Mask/1.28.3 Chrome/87.0.4280.141 Electron/11.4.5 Safari/537.36`;
